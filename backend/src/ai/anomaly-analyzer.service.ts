...
export class AnomalyAnalyzerService {
  ...
  /**
   * Analyze non-conforme tournées for a date range and identify root causes using Gemini AI.
   */
  async analyzeNonConformites(dateFrom?: string, dateTo?: string): Promise<AnomalyAnalysisResult> {
...
Return ONLY valid JSON, no markdown.`;
    ...
    try {
      const result = await this.gemini.generateJSON<Omit<AnomalyAnalysisResult, 'totalNonConforme' | 'period'>>(
        prompt,
        systemInstruction,
      );
  ...
  /**
   * Send analysis report via email to the configured recipient.
   */
  async sendReport(analysis: AnomalyAnalysisResult, recipientEmail?: string): Promise<{ sent: boolean; to: string }> {
    const to = recipientEmail || this.config.get<string>('ANOMALY_REPORT_EMAIL') || 'ghardadounadhir306@gmail.com';
  ...
  private async fetchAnomalyData(dateFrom?: string, dateTo?: string): Promise<any[]> {
    try {
      let sql = `
  ...
  private async fetchNonConformeFormData(dateFrom?: string, dateTo?: string): Promise<any[]> {
    try {
      let sql = `
  ...
  private buildAnalysisPrompt(anomalies: any[], formData: any[], dateFrom?: string, dateTo?: string): string {
    const period = `${dateFrom || 'début'} → ${dateTo || 'maintenant'}`;
    const total = anomalies.length + formData.length;

    // Summarize anomaly categories from transport_data
    const missingDriver = anomalies.filter((a: any) => !a.driver || a.driver === '').length;
    const missingTruck = anomalies.filter((a: any) => !a.truck || a.truck === '').length;
    const zeroPalettes = anomalies.filter((a: any) => !a.palettes || a.palettes === 0).length;
    const pendingState = anomalies.filter((a: any) => a.states === 'pending').length;

    // Driver issues
    const driverIssues: Record<string, number> = {};
    for (const a of anomalies) {
      if (a.driver) driverIssues[a.driver] = (driverIssues[a.driver] || 0) + 1;
    }

    // Site issues
    const siteIssues: Record<string, number> = {};
    for (const a of anomalies) {
      if (a.site) siteIssues[a.site] = (siteIssues[a.site] || 0) + 1;
    }

    // Non-conforme form data
    const conformiteReasons: Record<string, number> = {};
...
Analyse les causes profondes, identifie les patterns récurrents, et propose des actions correctives concrètes.`;
  ...
  private buildEmailHtml(analysis: AnomalyAnalysisResult): string {
