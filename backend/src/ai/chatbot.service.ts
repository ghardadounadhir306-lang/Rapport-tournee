...
/** Schema description passed to Gemini so it can write SQL for any question. */
...
export class ChatbotService {
   ...
   * 1) Try fast local handlers for common questions
   * 2) Fall back to Gemini AI to generate SQL and answer any question from the data
   */
  async chat(message: string, history: ChatMessage[] = []): Promise<ChatResponse> {
    const detectedLang = this.detectLanguage(message);

    // 1) Try local handlers (fast, no AI call needed)
    const localAnswer = await this.tryLocalAnswer(message, detectedLang);
    if (localAnswer) {
      return localAnswer;
    }

    // 2) Try Gemini-powered answer (can answer any question from the data)
    ...
    // 3) Fallback when Gemini is unavailable
  ...
  /**
   * Use Gemini to understand the user's question, generate a SQL query,
   * execute it, and return a natural language answer with data.
   */
...
Analyze the question and respond appropriately. If it's a data question, generate a SQL query. If it's a general knowledge question, answer directly.`;
    ...
    try {
      ...
      // Try to extract a useful response anyway
      return {
    ...
    // If Gemini says the question can't be answered from the DB
    if (!parsed.sql) {
      return {
    ...
    try {
      ...
      // Build the natural language reply
      ...
      if (!rows || rows.length === 0) {
        const noDataReplies: Record<string, string> = {
  ...
  private getFallbackReply(lang: string): string {
    const replies: Record<string, string> = {
  ...
  private detectLanguage(text: string): string {
    ...
    if (darijaPatterns.test(lower)) return 'ar';
  ...
  // ── Local handlers (fast, no AI call) ───────────────────────────────────

  private isDriverAnomalyQuestion(message: string): boolean {
    ...
    return (
      anomalyWords.some((word) => normalized.includes(word)) &&
      driverWords.some((word) => normalized.includes(word)) &&
      rankingWords.some((word) => normalized.includes(word))
    );
  }

  private isConformiteCountQuestion(message: string): boolean {
    const normalized = this.normalizeText(message);
    ...
    return (
      countWords.some((w) => normalized.includes(w)) &&
      conformiteWords.some((w) => normalized.includes(w))
    );
  }

  private getConformiteFilter(message: string): 'non_conforme' | 'conforme' | 'all' {
    const normalized = this.normalizeText(message);
  ...
  private async tryLocalAnswer(message: string, detectedLang: string): Promise<ChatResponse | null> {
    ...
    try {
      const rows = await this.dataSource.query(
        `SELECT COALESCE(NULLIF(TRIM(driver), ''), 'Non renseigné') AS driver,
  ...
  private async answerConformiteCount(message: string, detectedLang: string): Promise<ChatResponse> {
    const filter = this.getConformiteFilter(message);

    try {
      const rows = await this.dataSource.query(
        `SELECT COALESCE(NULLIF(TRIM(conformite), ''), 'Non renseigné') AS conformite,
                COUNT(*)::int AS total
         FROM tms_form_data
      ...
      const grandTotal = rows.reduce((sum: number, r: any) => sum + (Number(r.total) || 0), 0);

      if (filter === 'non_conforme') {
        const nonConformeRows = rows.filter(
          (r: any) => (r.conformite || '').toLowerCase().trim() !== 'conforme' && r.conformite !== 'Non renseigné',
        );
        const nonConformeTotal = nonConformeRows.reduce((sum: number, r: any) => sum + (Number(r.total) || 0), 0);
      ...
      if (filter === 'conforme') {
        const conformeRow = rows.find(
          (r: any) => (r.conformite || '').toLowerCase().trim() === 'conforme',
        );
      ...
      return null as any; // Fall through to Gemini
    }
  }
}
