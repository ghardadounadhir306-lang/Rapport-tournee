...
export class PredictionService {
  ...
  /**
   * Predict delays for pending/upcoming tournées.
   * Analyzes historical patterns for similar driver/truck/route combinations.
   */
  async predictDelays(tourneeIds?: string[]): Promise<PredictionResult[]> {
    // If specific IDs are requested, handle individually (small batch)
    ...
    // Bulk mode: fetch all driver stats in a single SQL query
    return this.predictAllDriversBulk();
  }

  /**
   * Bulk prediction for ALL drivers using pure SQL + statistical scoring.
   * No individual Gemini calls — fast and works for 300+ drivers.
   */
  private async predictAllDriversBulk(): Promise<PredictionResult[]> {
    try {
      ...
      const predictions: PredictionResult[] = rows.map((row: any) => {
        ...
        // Low conformity → higher risk
        if (conformityPct < 40) riskScore += 40;
        else if (conformityPct < 60) riskScore += 30;
        else if (conformityPct < 75) riskScore += 20;
        else if (conformityPct < 90) riskScore += 10;

        // Many non-conforme records → higher risk
        if (nonConformeCount > 20) riskScore += 30;
        else if (nonConformeCount > 10) riskScore += 20;
        else if (nonConformeCount > 5) riskScore += 15;
        else if (nonConformeCount > 2) riskScore += 8;

        // Non-conforme ratio
        const ncRatio = nonConformeCount / totalTours;
        if (ncRatio > 0.5) riskScore += 20;
        else if (ncRatio > 0.3) riskScore += 12;
        else if (ncRatio > 0.15) riskScore += 6;
        ...
        if (totalTours === 0) {
          ...
          // Low risk (score < 25) — good performance
          if (conformityPct >= 95) {
        ...
        // Cross-cutting recommendations applicable regardless of level
  ...
  private async predictSingleWithAI(tournee: any): Promise<PredictionResult | null> {
    try {
      const stats = await this.getHistoricalStats(
        tournee.driver,
        tournee.truck,
        tournee.site_id || tournee.sitcode,
      );
...
Return ONLY this JSON:
{
  ...
  "recommendations": ["string in French"]
      ...
      const aiResult = await this.gemini.generateJSON<{
        riskScore: number;
        riskLevel: string;
        predictedDelayMin: number;
      ...
      return {
        ...
        date: tournee.date || '',
        riskLevel: (aiResult.riskLevel as PredictionResult['riskLevel']) || this.scoreToLevel(baseScore),
  ...
  private async fetchPendingTournees(ids?: string[]): Promise<any[]> {
    try {
      if (ids && ids.length > 0) {
        const placeholders = ids.map((_, i) => `$${i + 1}`).join(',');
        return await this.dataSource.query(
          `SELECT id, date, wms, truck, driver, conformite, site_id, prestation_id,
      ...
      // Fetch the most recent tournée per unique driver (covers all chauffeurs)
      return await this.dataSource.query(`
  ...
  private async getHistoricalStats(
    ...
    driverConformityPct: number;
    driverAvgDelayMin: number;
    driverAnomalyCount: number;
    truckConformityPct: number;
    truckAnomalyCount: number;
    routeAvgDurationMin: number;
    routeKmVariancePct: number;
  }> {
    const defaults = {
    ...
    try {
      ...
      if (driver) {
        ...
        if (driverRows.length > 0) {
          const conforme = driverRows.filter((r: any) =>
            (r.conformite || '').toLowerCase().trim() === 'conforme'
          ).length;
          defaults.driverConformityPct = Math.round((conforme / driverRows.length) * 100);

          // Calculate average duration
          const durations = driverRows
            .map((r: any) => this.timeDiffMinutes(r.h_depart, r.h_retour))
            .filter((d: number) => d > 0);
      ...
      if (truck) {
        ...
        if (truckRows.length > 0) {
          const conforme = truckRows.filter((r: any) =>
            (r.conformite || '').toLowerCase().trim() === 'conforme'
          ).length;
      ...
      if (siteId) {
        ...
        if (routeRows.length > 0) {
          ...
          if (kms.length > 0) {
            const avg = kms.reduce((a: number, b: number) => a + b, 0) / kms.length;
            const variance = kms.reduce((a: number, b: number) => a + Math.pow(b - avg, 2), 0) / kms.length;
          ...
          const durations = routeRows
            ...
            .filter((d: number) => d > 0);
          if (durations.length > 0) {
            defaults.routeAvgDurationMin = Math.round(
  ...
  private calculateStatisticalRisk(stats: ReturnType<typeof this.getHistoricalStats> extends Promise<infer T> ? T : never): number {
    let score = 0;

    // Low driver conformity → higher risk
    if (stats.driverConformityPct < 60) score += 30;
    else if (stats.driverConformityPct < 75) score += 20;
    else if (stats.driverConformityPct < 90) score += 10;

    // Many anomalies → higher risk
    if (stats.driverAnomalyCount > 10) score += 25;
    else if (stats.driverAnomalyCount > 5) score += 15;
    else if (stats.driverAnomalyCount > 2) score += 8;

    // Low truck conformity
    if (stats.truckConformityPct < 60) score += 20;
    else if (stats.truckConformityPct < 75) score += 12;
    else if (stats.truckConformityPct < 90) score += 5;

    // High route variance
    if (stats.routeKmVariancePct > 30) score += 15;
    else if (stats.routeKmVariancePct > 15) score += 8;

    // Truck anomalies
    if (stats.truckAnomalyCount > 5) score += 10;

    return Math.min(100, score);
  }

  private scoreToLevel(score: number): PredictionResult['riskLevel'] {
