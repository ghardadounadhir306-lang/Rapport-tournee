...
export class TmsService {
  constructor(
    ...
    private readonly tmsFormDataRepo: Repository<TmsFormData>,
  ) {}

  async saveFormData(tmsId: string, tableRows: any, inputData: any) {
    if (!tmsId) {
      throw new BadRequestException('tmsId is required');
    }
    let formData = await this.tmsFormDataRepo.findOne({ where: { tms_id: tmsId } });
    if (formData) {
      formData.table_rows = tableRows;
      formData.input_data = inputData;
      return await this.tmsFormDataRepo.save(formData);
    } else {
      const newFormData = this.tmsFormDataRepo.create({
        id: uuidv4(),
        tms_id: tmsId,
        table_rows: tableRows,
        input_data: inputData,
      });
      return await this.tmsFormDataRepo.save(newFormData);
    }
  }

  async getFormData(tmsId: string) {
    return await this.tmsFormDataRepo.findOne({ where: { tms_id: tmsId } });
  }

  async create(data: Partial<TmsImportRow>) {
    const newEntry = this.tmsImportRowRepo.create(data);
    return await this.tmsImportRowRepo.save(newEntry);
  }

  async update(id: string, data: Partial<TmsImportRow>) {
    await this.tmsImportRowRepo.update(id, data);
    return this.tmsImportRowRepo.findOne({ where: { id } });
  }
