...
let TmsService = class TmsService {
    ...
    formDataRepo;
    dataSource;
    constructor(tmsImportRowRepo, formDataRepo, dataSource) {
    ...
    async fetchTransportDataRows(limit) {
        try {
            const rows = await this.dataSource.query(`SELECT ROW_NUMBER() OVER (ORDER BY otsnum DESC NULLS LAST) AS _rn,
    ...
    async getTransportData(rawLimit) {
        ...
        const safeLimit = Number.isFinite(parsedLimit) && parsedLimit > 0
            ...
            : 100;
        try {
            const rows = await this.dataSource.query(`SELECT * FROM transport_data LIMIT ${safeLimit}`);
            return {
    ...
    filterListByQuery(list, query) {
        ...
        return list.filter((item) => {
            ...
            if (q('site') && !(item.site ?? '').toLowerCase().includes(q('site')))
                return false;
