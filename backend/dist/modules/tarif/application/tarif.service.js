...
exports.TarifService = void 0;
const common_1 = require("@nestjs/common");
const tarifsData_1 = require("../infrastructure/data/tarifsData");
const storesData_1 = require("../infrastructure/data/storesData");
const flegData_1 = require("../infrastructure/data/flegData");
const diversData_1 = require("../infrastructure/data/diversData");
const tractageData_1 = require("../infrastructure/data/tractageData");
const clientDiversData_1 = require("../infrastructure/data/clientDiversData");
const freezerData_1 = require("../infrastructure/data/freezerData");
let TarifService = class TarifService {
    getStores() {
        return storesData_1.azizaStores;
    }
    calculateTarif(dto) {
        ...
        if (km == null || (!hasStores && palettes == null) || !nature) {
            throw new common_1.BadRequestException('km, palettes et nature sont obligatoires');
        }
        if (hasStores) {
            const selectedStoreData = stores.map(s => storesData_1.azizaStores.find(db => db.name === s.name));
            ...
            if (foundStores.length > 0) {
                ...
                if (nature !== 'Fleg' && nature !== 'Divers' && isMultiSector && tourneeType === 'Generique') {
                    throw new common_1.BadRequestException('تنبيه: لا يمكن أن تكون الدورة "Generique" لأن المغازات المحددة تنتمي لقطاعات مختلفة (Sectors). يرجى تغيير نوع الدورة إلى "Non Generique".');
                }
            }
        }
        ...
        if (nature === 'Fleg') {
            ...
            tarifRaw = basePrice + (Number(km) * flegData_1.flegConfig.kmRate);
            ...
            if (!matchedRow) {
                throw new common_1.NotFoundException('Aucune ligne tarifaire correspondante');
            }
    ...
    findTarifRow({ km, palettes, nature }) {
        const kmNum = Number(km);
        const palNum = Number(palettes);
        return tarifsData_1.tarifsTable.find((r) => {
            if (r.Type !== nature)
                return false;
            ...
            return (Number.isFinite(distMin) &&
    ...
    getCapacityMeta(row) {
        ...
        if (!type || !Number.isFinite(capMin) || !Number.isFinite(capMax))
            return null;
        const reference = tarifsData_1.tarifsTable.find((r) => {
            if (r.Type !== type)
                return false;
        ...
        if (!reference)
            return null;
        const meta = {};
        if (reference.Nature != null)
            meta.Nature = reference.Nature;
        if (reference['Capcité'] != null)
            meta['Capcité'] = reference['Capcité'];
        if (reference.R25 != null)
            meta.R25 = reference.R25;
        if (reference.R50 != null)
            meta.R50 = reference.R50;
        if (reference.R75 != null)
            meta.R75 = reference.R75;
        if (reference.R100 != null)
            meta.R100 = reference.R100;
        if (reference.R125 != null)
            meta.R125 = reference.R125;
        return Object.keys(meta).length > 0 ? meta : null;
    }
    computeRemiseKeys(roundTripKm, { majoration = false } = {}) {
        let baseKey;
        if (roundTripKm <= 25)
            baseKey = 'R25';
        else if (roundTripKm <= 50)
            baseKey = 'R50';
        else if (roundTripKm <= 75)
            baseKey = 'R75';
        else if (roundTripKm <= 100)
            baseKey = 'R100';
        else
            baseKey = 'R125';
        let key = null;
    ...
    pickRemise(row, roundTripKm, { majoration = false } = {}) {
        if (!row || !Number.isFinite(roundTripKm))
            return 0;
        ...
        if (typeof value !== 'number') {
            ...
            const capMin = Number(row.CapMin);
            const capMax = Number(row.CapMax);
            const fallback = tarifsData_1.tarifsTable.find((r) => {
                if (r.Type !== type)
                    return false;
                const dMin = Number(r.DistMin);
                const dMax = Number(r.DistMax);
        ...
        switch (diversCategory) {
            case 'diversAziza': {
                const getBaseTarif = () => {
                    ...
                    if (!baseRow) {
                        throw new common_1.NotFoundException('Aucune ligne tarifaire Aziza correspondante pour ce km/palettes');
                    }
                ...
                switch (diversSubCategory) {
                    ...
                    case 'transfertRetour':
                        {
                            ...
                            const zoneTarif = diversData_1.diversData?.diversAziza?.transfertRetour?.zones?.[zoneKey];
                            const palettesNum = Number(palettes) || 0;
                            if (!zoneKey || !Number.isFinite(Number(zoneTarif))) {
                                throw new common_1.BadRequestException('Zone Tarif invalide pour Transfert Retour (trp13).');
                            }
                    ...
                    case 'tarifAnexe':
                        {
                            ...
                            if (!Number.isFinite(tarifAnexeNum) || tarifAnexeNum <= 0) {
                                throw new common_1.BadRequestException('Tarif Annexe invalide (trp33).');
                            }
                    ...
                    case 'tarifOct':
                        ...
                        {
                            const zoneKey = typeof dto.zone === 'string' ? dto.zone : '';
                            const zoneTarif = diversData_1.diversData?.diversAziza?.tarifOct?.zones?.[zoneKey];
                            if (!zoneKey || !Number.isFinite(Number(zoneTarif)) || Number(zoneTarif) <= 0) {
                                throw new common_1.BadRequestException('Tarif OCT (trp10): Zone OCT invalide ou tarif non configuré.');
                            }
                    ...
                    case 'transportSurAchat':
                        {
                            ...
                            const rate = diversData_1.diversData?.diversAziza?.transportSurAchat?.tauxKm?.[vehicleKey];
                            if (!Number.isFinite(kmNum) || kmNum <= 0) {
                                throw new common_1.BadRequestException('Rayon (km) invalide pour Transport sur Achat (trp6).');
                            }
                            if (vehicleKey !== 'cargo' && vehicleKey !== 'semi') {
                                throw new common_1.BadRequestException('Type véhicule invalide pour Transport sur Achat (trp6).');
                            }
                            if (!Number.isFinite(Number(rate)) || Number(rate) <= 0) {
                                throw new common_1.BadRequestException('Taux Km non configuré pour Transport sur Achat (trp6).');
                            }
                    ...
                    case 'transfertInterDepotSpot':
                        {
                            ...
                            if (!Number.isFinite(kmNum) || kmNum <= 0) {
                                throw new common_1.BadRequestException('Rayon (km) invalide pour Transfert Inter Dépôt Spot (trp15).');
                            }
                            if (!Number.isFinite(palettesNum) || palettesNum <= 0) {
                                throw new common_1.BadRequestException('Nombre de palettes invalide pour Transfert Inter Dépôt Spot (trp15).');
                            }
                            const cfg = diversData_1.diversData?.diversAziza?.transfertInterDepotSpot;
                            ...
                            if (!Number.isFinite(Number(taux)) || Number(taux) <= 0) {
                                throw new common_1.BadRequestException('Taux non configuré pour Transfert Inter Dépôt Spot (trp15).');
                            }
                    ...
                    case 'transportSurgele':
                        {
                            const kmNum = Number(km) || 0;
                            if (!Number.isFinite(kmNum) || kmNum <= 0) {
                                throw new common_1.BadRequestException('Rayon (km) invalide pour Transport Surgelé.');
                            }
                            if (!Number.isFinite(besoinNum) || besoinNum <= 0) {
                                throw new common_1.BadRequestException('Besoin invalide pour Transport Surgelé.');
                            }
                            const tarifFraisAziza = Number(diversData_1.diversData?.diversAziza?.transportSurgele?.tarifFraisAziza);
                            const majorationPercent = Number(diversData_1.diversData?.diversAziza?.transportSurgele?.majorationSurgele);
                            if (!Number.isFinite(tarifFraisAziza) || tarifFraisAziza <= 0) {
                                throw new common_1.BadRequestException('Transport Surgelé: tarifFraisAziza non configuré.');
                            }
                            if (!Number.isFinite(majorationPercent) || majorationPercent < 0) {
                                throw new common_1.BadRequestException('Transport Surgelé: majorationSurgele invalide.');
                            }
                    ...
                    case 'transfertInterMagazin':
                        {
                            const baseTarif = getBaseTarif();
                            const retourPercentRaw = Number(diversData_1.diversData?.diversAziza?.transfertInterMagazin?.tarifs?.baseRetours);
                    ...
                    case 'transfertLilas':
                        {
                            const kmNum = Number(km) || 0;
                            if (!Number.isFinite(kmNum) || kmNum <= 0) {
                                throw new common_1.BadRequestException('Rayon (km) invalide pour Transfert Lilas (trp12).');
                            }
                            if (!Number.isFinite(besoinNum) || besoinNum <= 0) {
                                throw new common_1.BadRequestException('Nombre de véhicules invalide pour Transfert Lilas (trp12).');
                            }
                        ...
                        break;
                    default:
                        throw new common_1.BadRequestException('Sous-catégorie Divers Aziza invalide.');
                }
                break;
            }
            case 'industriesEtCsm':
                {
                    ...
                    if (!Number.isFinite(kmNum) || kmNum <= 0) {
                        throw new common_1.BadRequestException('Industries & CSM (trp23): rayon (km) invalide.');
                    }
                    if (!Number.isFinite(besoinNum) || besoinNum <= 0) {
                        throw new common_1.BadRequestException('Industries & CSM (trp23): nombre de véhicules invalide.');
                    }
                    if (!site) {
                        throw new common_1.BadRequestException('Industries & CSM (trp23): veuillez sélectionner le site (Zit/Wad Lil/Dandan/Migrin).');
                    }
                    const siteTable = tractageData_1.tractageSiteKmRate;
                    const pickedSiteRate = site === 'zit' ? siteTable?.zit :
                        (site === 'wad lil' || site === 'wadlil' || site === 'oued lil' || site === 'ouedlil') ? siteTable?.wadLil :
                            site === 'dandan' ? siteTable?.dandan :
                                (site === 'migrin' || site === 'mégrine' || site === 'megrine') ? siteTable?.migrin :
                                    undefined;
                    const defaultRate = Number(tractageData_1.tractageConfig?.defaultKmRate);
                    ...
                    if (!Number.isFinite(kmRateSite) || kmRateSite <= 0) {
                        throw new common_1.BadRequestException('Industries & CSM (trp23): kmRate(site) non configuré.');
                    }
                    let vehicleKey = null;
                    if (vehicle.includes('npr') && vehicle.includes('iveco'))
                        vehicleKey = 'nprIveco';
                    else if (vehicle.includes('pic'))
                        vehicleKey = 'picup';
                    else if (vehicle.includes('nkr'))
                        vehicleKey = 'nkr';
                    else if (vehicle.includes('npr'))
                        vehicleKey = 'npr';
                    else if (vehicle.includes('mercedes'))
                        vehicleKey = 'mercedes';
                    else if (vehicle.includes('semi'))
                        vehicleKey = 'semi';
                    else if (vehicle.includes('iveco'))
                        vehicleKey = 'iveco';
                    const defaultVehicleFactor = Number(tractageData_1.tractageConfig?.defaultVehicleFactor);
                    const factor = vehicleKey ? Number(tractageData_1.tractageVehicleFactor[vehicleKey]) : (Number.isFinite(defaultVehicleFactor) ? defaultVehicleFactor : 1);
                    if (!Number.isFinite(factor) || factor <= 0) {
                        throw new common_1.BadRequestException('Industries & CSM (trp23): factor(vehicule) non configuré.');
                    }
                    finalTarif = kmNum * besoinNum * kmRateSite * factor;
                    details = {
                        formula: 'rayon(km) * nb_vehicules * kmRate(site) * factor(vehicule)',
                        site: rawSite,
                        destination: tractageData_1.tractageConfig?.destinationLabel || 'Bouargoub',
                        km: kmNum,
                        nbVehicules: besoinNum,
                        vehicule: rawVehicle,
                        kmRateSite,
            ...
            case 'clientDivers':
                {
                    if (!Number.isFinite(besoinNum) || besoinNum <= 0) {
                        throw new common_1.BadRequestException('Client Divers: nombre de véhicules invalide.');
                    }
                    const rawZone = typeof dto.zone === 'string' ? dto.zone : '';
                    const zoneKey = rawZone.trim().toUpperCase() || clientDiversData_1.clientDiversConfig.defaultZone;
                    const rawVehicle = typeof dto.vehicule === 'string' ? String(dto.vehicule) : '';
                    const vehicle = rawVehicle.trim().toLowerCase();
                    let vehicleKey = null;
                    if (vehicle.includes('npr') && vehicle.includes('iveco'))
                        vehicleKey = 'nprIveco';
                        ...
                        vehicleKey = 'iveco';
                    if (!vehicleKey) {
                        vehicleKey = clientDiversData_1.clientDiversConfig.defaultVehicule;
                    }
                    const tarifZoneVehicule = Number(clientDiversData_1.clientDiversZoneVehiculeTarifs?.[zoneKey]?.[vehicleKey]);
                    const tarifZoneFallback = Number(clientDiversData_1.clientDiversZoneTarifs[zoneKey]);
                    ...
                    if (!Number.isFinite(tarifZone) || tarifZone <= 0) {
                        throw new common_1.BadRequestException('Client Divers: zone/vehicule invalide ou tarif non configuré.');
                    }
                    const brut = besoinNum * tarifZone;
                    const remisePercent = dto.isSameDepartureAndReturn ? clientDiversData_1.clientDiversConfig.remiseMemeLieuRetour : 0;
                    const remiseAmount = brut * remisePercent;
                    finalTarif = brut - remiseAmount;
                    details = {
            ...
            case 'vielavieGlace':
                if (dto.surgelaOption === 'mghiraa') {
                    ...
                    if (!vehicle || !vehicle.includes('cargo')) {
                        throw new common_1.BadRequestException('Vielavie Glace (trp4): véhicule doit être cargo (option Mghiraa).');
                    }
                    const r = Number(freezerData_1.freezerData?.mghiraa?.R);
                    if (!Number.isFinite(r) || r <= 0) {
                        throw new common_1.BadRequestException('Vielavie Glace (trp4): R non configuré.');
                    }
                    if (!Number.isFinite(besoinNum) || besoinNum <= 0) {
                        throw new common_1.BadRequestException('Vielavie Glace (trp4): besoin (nombre cargo) invalide.');
                    }
                    ...
                    const rawVehicle = typeof dto.vehicule === 'string' ? String(dto.vehicule) : '';
                    const vehicle = rawVehicle.trim().toLowerCase();
                    let freezerVehicleKey = null;
                    if (vehicle.includes('nkr'))
                        freezerVehicleKey = 'nkr';
                    else if (vehicle.includes('npr'))
                        freezerVehicleKey = 'npr';
                    else if (vehicle.includes('cargo'))
                        freezerVehicleKey = 'cargo';
                    if (!freezerVehicleKey) {
                        throw new common_1.BadRequestException('Vielavie Glace (trp4): véhicule invalide (autres clients: Cargo/NKR/NPR فقط).');
                    }
                    const kmNum = Number(dto.km) || 0;
                    if (!Number.isFinite(kmNum) || kmNum <= 0) {
                        throw new common_1.BadRequestException('Vielavie Glace (trp4): rayon(km) invalide (autres clients).');
                    }
                    const maxKm = Number(freezerData_1.freezerData?.autresClients?.maxKm) || 500;
                    if (kmNum > maxKm) {
                        throw new common_1.BadRequestException(`Vielavie Glace (trp4): rayon(km) > ${maxKm} غير مدعوم.`);
                    }
                    const bucket = Math.ceil(kmNum / 25) * 25;
                    const tarifFreezer = Number(freezerData_1.freezerData?.autresClients?.tarifsFreezerByKm?.[freezerVehicleKey]?.[bucket]);
                    const fraisStationnement = Number(freezerData_1.freezerData?.autresClients?.fraisStationnement);
                    if (!Number.isFinite(tarifFreezer) || tarifFreezer <= 0) {
                        throw new common_1.BadRequestException('Vielavie Glace (trp4): tarif freezer(km) non configuré.');
                    }
                    if (!Number.isFinite(fraisStationnement) || fraisStationnement < 0) {
                        throw new common_1.BadRequestException('Vielavie Glace (trp4): frais stationnement non configuré.');
                    }
            ...
            case 'surgele': {
                ...
                const kmNum = Number(km) || 0;
                if (!Number.isFinite(kmNum) || kmNum <= 0) {
                    throw new common_1.BadRequestException('Surgelé (trp8): rayon(km) invalide.');
                }
                const factor = Number(diversData_1.diversData?.surgele?.zoneFactor?.[zoneTarifStr]);
                const baseFraisRate = Number(diversData_1.diversData?.diversAziza?.transportSurgele?.tarifFraisAziza);
                if (Number.isFinite(factor) && factor > 0 && Number.isFinite(baseFraisRate) && baseFraisRate > 0) {
                    ...
                    const legacyRate = Number(diversData_1.diversData?.surgele?.zones?.[zoneTarifStr]) || 10;
            ...
            default:
                throw new common_1.BadRequestException('Catégorie Divers non reconnue.');
        }
...
exports.TarifService = TarifService;
exports.TarifService = TarifService = __decorate([
    (0, common_1.Injectable)()
], TarifService);
//# sourceMappingURL=tarif.service.js.map