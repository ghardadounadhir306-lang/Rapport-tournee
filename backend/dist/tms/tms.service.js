...
let TmsService = class TmsService {
    tmsImportRowRepo;
    tmsFormDataRepo;
    constructor(tmsImportRowRepo, tmsFormDataRepo) {
        this.tmsImportRowRepo = tmsImportRowRepo;
        this.tmsFormDataRepo = tmsFormDataRepo;
    }
    async saveFormData(tmsId, tableRows, inputData) {
        if (!tmsId) {
            throw new common_1.BadRequestException('tmsId is required');
        }
