...
function App() {
  ...
  const handleSave = useCallback(async () => {
    try {
      // Gather inputs without changing UI
      const inputs = Array.from(document.querySelectorAll('input'));
      const inputData = {};
      inputs.forEach((input, index) => {
        if (input.name) {
          inputData[input.name] = input.type === 'checkbox' ? input.checked : input.value;
        } else {
          inputData[`input_${index}`] = input.type === 'checkbox' ? input.checked : input.value;
        }
      });
      ...
      const payload = {
        tableRows,
        inputData,
        selectedItem: selectedTmsItem,
        tmsId: selectedTmsId
      };

      const url = 'http://localhost:3000/tms' + (selectedTmsId ? `/${selectedTmsId}` : '');
      const method = selectedTmsId ? 'PUT' : 'POST';
  ...
  }, [tableRows, selectedTmsItem, selectedTmsId]);

  // Preset table rows based on selected tournée ID
  useEffect(() => {
    if (activeTab === 'AZIZA') {
      if (selectedTmsItem) {
        // Fetch saved form data from the database
        fetch(`http://localhost:3000/tms/form/${selectedTmsItem.id}`)
          .then(res => res.json())
          .then(data => {
            if (data && data.table_rows && data.table_rows.length > 0) {
              // Restore table rows and inputs
              setTableRows(data.table_rows);
              
              // We need a small timeout to let the DOM render the restored rows before setting inputs
              setTimeout(() => {
                if (data.input_data) {
                  const inputs = Array.from(document.querySelectorAll('input'));
                  inputs.forEach((input, index) => {
                    const key = input.name || `input_${index}`;
                    if (data.input_data[key] !== undefined) {
                      if (input.type === 'checkbox') {
                        input.checked = data.input_data[key];
                      } else {
                        input.value = data.input_data[key];
                      }
                    }
                  });
                }
        ...
        {activeTab === 'AZIZA' && (hasSelectedTournee ? (
          <section className={`content ${theme === 'dark' ? 'dark-theme-content' : 'light-theme-content'}`} style={{ padding: '20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '16px' }}>
              ...
              <button className={theme === 'dark' ? 'dark-save-btn' : 'light-save-btn'} onClick={handleSave}>Enregistrer</button>
            </div>
        ...
        {activeTab === 'DIVERS' && (hasSelectedTournee ? (
          <section className="content">
            ...
            <div className="flex justify-end gap-3 mt-4">
              ...
              <button className="btn-primary px-8 py-2 rounded-lg bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold shadow-lg shadow-orange-200 hover:scale-105 active:scale-95 transition-all" onClick={handleSave}>ENREGISTRER LA TOURNÉE</button>
            </div>
          </section>
        ) : (
          <section className="content">
